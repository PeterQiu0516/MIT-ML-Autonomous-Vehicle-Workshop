...
def LeNet(x):    
    ...
    conv2 = tf.nn.max_pool(conv2, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], name="conv_layer_2", padding='VALID')
    ...
    # Activation - relu.
    ##############################################################
    fc1_W = tf.Variable(tf.truncated_normal(shape=(int(fc0.shape[-1]), 120), mean = mu, stddev = sigma))
    ...
    logits = tf.add(tf.matmul(fc2_drop, fc3_W), fc3_b, name="logits_op") # with drop-out
...
prediction = tf.argmax(probs, 1, name="prediction_op")
